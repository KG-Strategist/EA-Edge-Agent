...
export default function DistillationTerminal() {
  ...
  useEffect(() => {
    ...
    const handleDistillationEvent = (e: Event) => {
      const { type, message, progress, recordId, error } = (e as CustomEvent).detail;
      let logText = '';
      if (type === 'STATUS') logText = message;
      else if (type === 'PROGRESS') logText = `Progress: ${Math.round(progress.progress || 0)}%`;
      else if (type === 'SUCCESS') logText = `Success! Record ID: ${recordId}`;
      else if (type === 'ERROR') logText = `Error: ${error}`;
      
      if (logText) {
        setLogs(prev => [...prev, {
          timestamp: new Date().toLocaleTimeString('en-US', { hour12: false }),
          direction: 'RX',
          agentId: 'DISTILLATION_WORKER',
          message: logText
        }]);
      }
    };

    window.addEventListener('DISTILLATION_EVENT', handleDistillationEvent);

    return () => {
      channel.close();
      window.removeEventListener('DISTILLATION_EVENT', handleDistillationEvent);
    };
  }, []);
    ...
    <div className="bg-black rounded-lg border border-gray-800 overflow-hidden shadow-2xl flex flex-col h-80">
      <div className="bg-gray-900 px-4 py-2 border-b border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-green-500 font-mono text-xs font-bold tracking-widest">DISTILLATION_TERMINAL_v1.0</span>
        </div>
