...
import { CheckCircle2, AlertTriangle, MonitorPlay, Activity, Plus, Edit, Archive as ArchiveIcon, ToggleLeft, ToggleRight, Loader2, X, Zap } from 'lucide-react';
...
import { resolveWasmUrl, SUPPORTED_MLC_MODELS } from '../../lib/constants';
...
export default function AgentConfigTab() {
  ...
  const offlineModels = useLiveQuery(() => db.model_registry.toArray()) || [];
      ...
      {config.engineType === 'WebLLM (Browser Cache)' && (
         <>
             ...
             {config.modelSourceMode === 'Remote URL' ? (
                <>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Model Registry</label>
                    <select
                    ...
                    <div className="flex gap-2">
                      ...
                      {(() => {
                        ...
                        const isLocked = isDirty || isThisDownloading;
                        return (
                          <button
                            onClick={() => handlePullWebCache(config.id, config.url)}
                            disabled={isLocked}
                            aria-label="Pull Web Cache"
                            title={isDirty ? 'Please save configuration first.' : isThisDownloading ? 'Download in progress...' : 'Pull Web Cache'}
                            className={`text-xs px-3 py-2 rounded-lg transition-all font-bold uppercase flex items-center gap-1 whitespace-nowrap ${isThisDownloading ? 'bg-blue-600 text-white opacity-70 cursor-not-allowed animate-pulse' : isLocked ? 'bg-gray-200 text-gray-400 dark:bg-gray-800 dark:text-gray-600 opacity-50 cursor-not-allowed' : 'bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-500/20 dark:text-blue-400 dark:hover:bg-blue-500/30'}`}
                          >
  ...
  return (
    <div className="flex flex-col h-full space-y-6 overflow-y-auto">
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 flex items-start gap-3 shrink-0">
        <MonitorPlay className="text-blue-500 mt-0.5 shrink-0" size={20} />
        <div className="text-sm text-blue-800 dark:text-blue-300">
          <strong className="block mb-1 font-semibold">Agent Configurations & Personas</strong>
          Configure local LLM routing parameters and strict persona governance boundaries.
        </div>
      </div>
