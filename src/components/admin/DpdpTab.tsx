...
import { ShieldAlert, Trash2, AlertTriangle, Plus, ToggleLeft, ToggleRight, Shield, X } from 'lucide-react';
import { db, PrivacyGuardrail } from '../../lib/db';
...
export default function DpdpTab() {
  ...
  const handleAddGuardrail = async () => {
    ...
    await db.privacy_guardrails.add({
      title: trimTitle,
      ruleText: trimRule,
      isDefault: false,
      isActive: true,
  ...
  const handleDelete = async (id: number) => {
    await db.privacy_guardrails.delete(id);
    loadGuardrails();
  };
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-6 mb-6">
        ...
        {guardrails.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 py-6 text-center">No guardrails configured. Add one to enforce compliance boundaries on AI output.</p>
        ) : (
            ...
            {guardrails.map(g => (
              <div key={g.id} className={`flex items-start gap-3 py-3 ${!g.isActive ? 'opacity-50' : ''}`}>
                ...
                <button
                  ...
                  title={g.isDefault ? 'Default policies cannot be deleted' : 'Delete guardrail'}
                >
                  <Trash2 size={15} />
                </button>
              </div>
