...
import MoESelector from './MoESelector';
...
export default function Header({ currentView, setCurrentView, adminSubView }: HeaderProps) {
  ...
  return (
    <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 transition-colors duration-300">
      <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 overflow-x-auto whitespace-nowrap hide-scrollbar">
        {breadcrumbs.map((crumb, index) => (
          <React.Fragment key={index}>
            {index > 0 && <ChevronRight size={14} className="mx-1 sm:mx-2 shrink-0" />}
            <button 
              onClick={crumb.onClick}
              className={`flex items-center hover:text-blue-600 dark:hover:text-blue-400 transition-colors ${index === breadcrumbs.length - 1 ? 'font-semibold text-gray-900 dark:text-gray-100' : ''}`}
