    ...
    if (type === 'INIT') {
      if (!db) {
        ...
        await db.exec(`
          ...
          CREATE TABLE IF NOT EXISTS bian_domains (
            id VARCHAR(10) PRIMARY KEY,
            name VARCHAR(255) NOT NULL,
        ...
        if (Number((res.rows[0] as any).count) === 0) {
          console.log('[PGLite] Seeding TOGAF and BIAN data...');
          // Seed TOGAF
          ...
          // Seed BIAN
          for (const domain of seedData.bian_domains) {
            await db.query('INSERT INTO bian_domains (id, name, businessArea, status) VALUES ($1, $2, $3, $4)', [domain.id, domain.name, domain.businessArea, domain.status]);
          }
        }
      }
