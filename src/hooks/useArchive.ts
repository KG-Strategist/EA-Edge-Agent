...
import { db, logForensicAudit } from '../lib/db';
...
export function useArchive({ tableName, statusField, archivedValue, activeValue, isRagEntity = false }: UseArchiveOptions) {
  ...
  const archiveItem = useCallback(async (identifier: number | string) => {
    ...
    if (isRagEntity && typeof identifier === 'string') {
      ...
      const existing = await table.get(identifier as number);
      if (existing) {
        const updated = { ...existing, [statusField]: archivedValue };
        await table.update(identifier as number, { [statusField]: archivedValue });
        await logForensicAudit('UPDATE', tableName, identifier, existing, updated);
      }
    }
    ...
    if (table) {
      if (isRagEntity && typeof identifier === 'string') {
        ...
        for (const job of jobs) {
           const updated = { ...job, [statusField]: activeValue };
           await db.training_jobs.update(job.id!, { [statusField]: activeValue } as any);
           await logForensicAudit('UPDATE', 'training_jobs', job.id!, job, updated);
        }
      } else {
        const existing = await table.get(identifier as number);
        if (existing) {
          const updated = { ...existing, [statusField]: activeValue };
  ...
  const permanentDeleteItem = useCallback(async (identifier: number | string) => {
    ...
    if (table) {
      if (isRagEntity && typeof identifier === 'string') {
         ...
         await db.training_jobs.bulkDelete(jobIds);
         for (const job of jobs) {
           await logForensicAudit('DELETE', 'training_jobs', job.id!, job, null);
         }
      } else {
         const existing = await table.get(identifier as number);
         if (existing) {
           await table.delete(identifier as number);
           await logForensicAudit('DELETE', tableName, identifier, existing, null);
         }
      }
    }
