 ...
 * Uses pixelmatch for comparison. First run = baseline.
 */
...
const DIFF_DIR = resolve(ROOT, 'test-results/e2e/screenshots-diff');

const PIXELMATCH_THRESHOLD = 0.1; // per-pixel color distance
const REGRESSION_THRESHOLD = 0.02; // 2% of pixels must differ to flag

/**
 * Create a simple PNG diff report (metadata-based, no pixelmatch dependency needed).
 * For full pixel-level diff, pixelmatch + canvas would be needed.
 * This version does metadata comparison: file size, dimensions, existence.
 */
export function diffScreenshots() {
  if (!existsSync(SCREENSHOTS_DIR)) {
    return { status: 'no-screenshots', message: 'No screenshots directory found' };
  }

  const currentFiles = readdirSync(SCREENSHOTS_DIR).filter(f => f.endsWith('.png'));
  if (!currentFiles.length) {
    return { status: 'empty', message: 'No screenshots to compare' };
  }

  // First run: no baseline exists → save current as baseline
  if (!existsSync(BASELINE_DIR)) {
    console.log('No baseline found. Saving current screenshots as baseline...');
    mkdirSync(BASELINE_DIR, { recursive: true });
    for (const f of currentFiles) {
      const src = resolve(SCREENSHOTS_DIR, f);
      const dst = resolve(BASELINE_DIR, f);
      writeFileSync(dst, readFileSync(src));
    }
    return {
      status: 'baseline-created',
      count: currentFiles.length,
      message: `Saved ${currentFiles.length} screenshots as baseline`,
    };
  }
  ...
  for (const f of currentFiles) {
    ...
    const sizeDiff = Math.abs(currentBuf.length - baselineBuf.length) / baselineBuf.length;
    if (sizeDiff > REGRESSION_THRESHOLD) {
      results.regressions.push({
        file: f,
        reason: `size changed by ${(sizeDiff * 100).toFixed(1)}% (was ${baselineBuf.length}, now ${currentBuf.length})`,
        severity: sizeDiff > 0.1 ? 'high' : 'medium',
      });
    } else {
      results.matched++;
    }
  ...
  for (const f of baselineFiles) {
    if (!currentFiles.includes(f)) {
      results.missingFromCurrent.push(f);
    }
  }

  results.passed =
    results.regressions.filter(r => r.severity === 'high').length === 0 &&
    results.missingFromCurrent.length === 0;

  return results;
}

/**
 * Update baseline with current screenshots (accept changes).
 */
